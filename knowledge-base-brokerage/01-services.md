# What We Build for Independent Broker-Dealers

Agentwerke builds custom AI agents for independent broker-dealers — the firms that recruit, supervise, and support 1099 reps. We focus on the operational and compliance work that drains your back office: onboarding, account opening, compliance review, and audit prep.

## Core offering: the Compliance Co-Pilot

Most BDs we work with start with a Compliance Co-Pilot. It's an agent that operates inside your compliance workflow — under your CCO's supervision — and handles the high-volume, repetitive review work that's drowning your team:

- **Compliance review queue triage** — pre-screens rep emails, social media posts, marketing materials, OBA disclosures, gift logs, and expense reports. Categorizes each item, flags risks, drafts approval or denial responses for human review. Compliance officers handle 3-5x more items per day.
- **Branch audit prep** — when FINRA gives you 30 days' notice on a branch audit, the agent compiles 18-24 months of records into the response package automatically. What used to be 40-60 hours of senior compliance time becomes a half-day review of an already-assembled package.
- **Customer complaint intake and routing** — logs new complaints within FINRA's reporting windows, classifies the complaint type, drafts the customer acknowledgment, and stages the FINRA reporting decision for human approval.
- **OBA disclosure review** — interviews reps about new outside business activities, drafts the disclosure, surfaces conflict-of-interest concerns for the supervising principal.

Every action by the agent is logged, every decision is staged for human approval before it leaves the firm. The agent never sends a regulatory filing on its own. It does the work; your principals approve and sign.

## Other agent types we build

### Onboarding Accelerator
For BDs that recruit 20+ reps per year. The agent:
- Walks new reps through document collection (U4 amendments, state registrations, OBAs)
- Drafts U4s and supporting docs from rep interviews
- Manages the 21-day bridge license workflow
- Surfaces gaps for compliance review before submission

For a firm bringing on 30 reps per year, this saves 250-450 hours of senior operations time annually.

### Rep Productivity Suite
For BDs trying to attract reps from wirehouses with better operational support. The agents:
- **New account opening assistant** — runs the rep through the client interview, fills the application, drafts the suitability narrative, manages ACATS and IRA rollover paperwork including DOL fiduciary documentation
- **Suitability documentation generator** — for any transaction over the firm's threshold, generates the written rationale that holds up in audits
- **Reg BI assistant** — produces the disclosures and care obligation documentation required for retail recommendations

### Continuing Education and Surveillance Support
- **CE tracker** — monitors each rep's rolling FINRA and firm-specific CE requirements, drafts personalized study guides, prompts study sessions
- **Branch supervisor copilot** — helps OSJ supervisors review their assigned reps' activity weekly, surfaces patterns

## What we don't do

- **Trade execution or order routing.** Operational risk is too high. We don't touch the order management system.
- **Rep email surveillance as a standalone product.** Smarsh, Global Relay, and Cisco own this market. We integrate with them but don't compete.
- **Robo-advisor or portfolio management.** Different regulatory regime (40 Act), different competitors, not our wedge.
- **Customer-facing chat for retail investors.** FINRA communications rules and fiduciary obligations make this complex. We can build internal-facing rep copilots; the customer-facing chat is a separate conversation.
- **Anything that bypasses the supervising principal.** Every output by an Agentwerke agent that has regulatory implications stages a human approval step. By design.

## Engagement model

Two-week discovery, 8-16 week build depending on tier, supervised launch with shadow mode for 30 days, then ongoing optimization. We integrate with your existing systems — we don't ask you to switch platforms. See the Process and Pricing docs for detail.
